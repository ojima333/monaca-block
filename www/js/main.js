...
var imgPath = {
    'ball' : 'img/ball.png',
    'ball_orange' : 'img/ball_orange.png',
    'ball_blue' : 'img/ball_blue.png',
    'ball_yellow' : 'img/ball_yellow.png',
    'ball_green' : 'img/ball_green.png',
...
var BB = {
    ...
    // Create blocks map
    setMap: function() {
        for(j = 0; j < maps[BB.level].length; j++) {
            for(i = 0; i < maps[BB.level][j].length; i++) {
                if(maps[BB.level][j][i] !== null) {
                    var block = BB.addBlock(10 + (30 * i), 80 + (12 * j), maps[BB.level][j][i]);
                }
            }
        }
    },
    ...
    addReserve: function(index) {
        ...
        ball.position.x = 100 + (index * 12);
        ball.position.y = BB.renderer.height - 25;
        
        ball.width = 10;
        ball.height = 10;
     
        BB.stage.addChild(ball);  
        BB.reserves.push(ball);
    },

    setReserve: function(num) {
        for (var i = 0; i < BB.reserves.length; i++) {
            if(i < num)
            {
                BB.reserves[i].alpha = 1.0;
            }
            else
            {
                BB.reserves[i].alpha = 0.0;
            }
        }
    },
...
//            'y' : 1.4 
    ...
    putFirework: function(x, y) {
var colors = ["ball_orange","ball_blue","ball_yellow","ball_green"];
var x = 80 + (Math.floor( Math.random() * 6 ) * 13);
var y = 100 + (Math.floor( Math.random() * 6 ) * 13);
var cindex = Math.floor( Math.random() * 4 );

        for(var i = 0; i < 10; i++) {

            var texture = PIXI.Texture.fromImage(imgPath[colors[cindex]], false);
            var ball = new PIXI.Sprite(texture);
...
var r = 5;
var a = i * 33;
var tx = r * Math.cos(a / 180 * Math.PI);  // X座標
var ty = r * Math.sin(a / 180 * Math.PI);  // Y座標
            ball.delta = {
                'x' : tx,
    //            'y' : -0.4 
    //            'y' : 1.4 
...
//        var ball = BB.balls.pop();
//        BB.accelLabel.setText(ball.delta.x);
    },
...
//        BB.accelLabel.setText(accelText);
    ...
    reset: function(score) {
        ...
        var ballLabel = new PIXI.Text("+BALL", {font: "24px/1.2 vt", fill: "blue"});
        ballLabel.position.x = 88;
        ballLabel.position.y = BB.renderer.height - 52;
        BB.stage.addChild(ballLabel);
        ballLabel.buttonMode = true;
        ballLabel.interactive = true;
        ballLabel.click = ballLabel.tap = function(data) {
            BB.pBall();
        };

        BB.nextLabel = new PIXI.Text("NEXT", {font: "24px/1.2 vt", fill: "green"});
        BB.nextLabel.position.x = 162;
        BB.nextLabel.position.y = BB.renderer.height - 52;
        BB.stage.addChild(BB.nextLabel);
        BB.nextLabel.buttonMode = false;
        BB.nextLabel.interactive = true;
        BB.nextLabel.alpha = 0.0;
        BB.nextLabel.click = BB.nextLabel.tap = function(data) {
            if(BB.nextLabel.alpha == 1.0) {
                BB.level++;
                BB.reset(BB.score);
            }
        };

        BB.clearLabel = new PIXI.Text("CLEARED", {font: "24px/1.2 vt", fill: "blue"});
        BB.clearLabel.position.x = 88;
        BB.clearLabel.position.y = BB.renderer.height - 352;
        BB.stage.addChild(BB.clearLabel);
        BB.clearLabel.alpha = 0.0;
        setTimeout(function() {
...
*/        
    ...
    clearGame: function() {
        ...
        {
            navigator.notification.alert("Cleared! defined", function(){}, "Congraturations");
        }
        else
        {
//            alert("Cleared! undefined");
            BB.clearLabel.alpha = 1.0;
        }
        if(BB.level < (maps.length - 1))
        {
            BB.nextLabel.alpha = 1.0;
            BB.nextLabel.buttonMode = true;
            BB.gameState = GAMESTATE_STOP;
        }
        else
        {
            BB.putFirework();
//            setTimeout(BB.putFirework, 1500);
//            setTimeout(BB.putFirework, 3000);
//            setTimeout(BB.putFirework, 4500);
        }
    }
}
...
//                BB.paddle.accel += (SETTINGS_PADDLE_ACCEL * 0.1);
                BB.paddle.accel = (SETTINGS_PADDLE_ACCEL * 4.1);
                break;
            case 39:
//                BB.paddle.position.x += 4;
//                BB.paddle.accel -= (SETTINGS_PADDLE_ACCEL * 0.1);
...
function animate() {
 ...
 //                   if (BB.balls.length <= 0) {
                        BB.clearLabel.alpha = 1.0;
                        BB.clearLabel.setText("GAME OVER");
                        BB.endGame();
                    }
                }
                continue;
